...
## check RFE results; 
rfe_rf 
plot_ROC(rfe_rf, " genes, with random-forest RFE", NULL)
plot_ROC(rfe_rf, " genes, with random-forest RFE", 9) 
printOptSet(9, rfe_rf)


rfe_knn <- rfe(t(count), progress$Progression.Logit, sizes=c(1:49),
              method="knn",
              rfeControl = ctrl, 
              metric = "ROC", 
              trControl = trainctrl,
...
plot_ROC(rfe_knn, " genes, with KNN-RFE", NULL)

##########################################################################
## this step might take ~120 mins, depends on the hardware of the computer
set.seed(100)
rfe_svm <- rfe(t(count), progress$Progression.Logit, sizes=c(1:49),
              method="svmRadial",
              rfeControl = ctrl, 
              metric = "ROC", 
              trControl = trainctrl,
...
rfe_ada <- rfe(t(count), progress$Progression.Logit, sizes=c(1:49),
               method="ada",
               rfeControl = ctrl, 
               metric = "ROC", 
            #   trControl = trainctrl,
...
plot_ROC(rfe_ada, " Genes, ada-RFE model opt", NULL)

# plot the optimized ROC
plot_ROC(rfe_svm, " Genes, svm-RFE model opt", NULL)

plot_ROC(rfe_svm, " Genes, svm-RFE model opt", 2)

plot_ROC(rfe_svm, " svm-RFE model", 39)
printOptSet(2, rfe_svm)

plot_ROC(rfe_svm, " svm-RFE model", 20) 
printOptSet(6, rfe_svm)
printOptSet(109, rfe_svm)
...
} ## end printOptSet() function;

# set.size = 13 # suppoe you want set-size of 10

rfe.vars <- rfe_svm$variables 
