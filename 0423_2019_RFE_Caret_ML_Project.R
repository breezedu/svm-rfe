...
count <- read.table("D:/WorkRecord/Companies/Qiagen_Sales/201904/test_miRNA/miRNA_LUAD_TCGAB37_Counts.txt", row.names = 1, header = T, sep = "\t")

design <- read.table("D:/WorkRecord/Companies/Qiagen_Sales/201904/test_miRNA/miRNA_LUAD_TCGAB37_Design.txt", row.names = 1, header = T, sep = "\t")

## for miRNA from TCGA, create a new column 'class' based on sample.type
summary(design$Sample.Type)

design$class <- with(design, ifelse(Sample.Type == "Primary Tumor", "Tumor", "Normal"))

summary(design$class)

## assign factor levels: tumor or normal
design$class <- as.factor(design$class)
...
top.genes <- readLines("D:/WorkRecord/Companies/Qiagen_Sales/201904/test_miRNA/significant_genes_LUAD_miRNA_DESeq2.txt")
...
trainRowNumbers <- createDataPartition(my_data$class, p=0.7, list=FALSE)
...
featurePlot(x = trainData[, 1:20], 
            ...
            ) # end featurePlot() with box expression;
...
ctrl <- rfeControl(functions = rfFuncs,
                   ...
                   number = 5,                ## fold of cross validation
...
lmProfile <- rfe(x=trainData[, 1:423], 
                 y=trainData$class,
...
## plot smoothed ROC with labels and legends and AUC
plot(
  
  smooth( rocobj_svmlinear ),
  #ci=TRUE,
  legacy.axes=TRUE, percent=TRUE,
  main = "svmLinear",
  xlab="False Positive Percentage", 
...
PlotROCSaveJPG(rocobj_svmlinear, path, "svmLinear_tune5", "svmLinear" )
...
### save the ROC with svmRadiacl plot to local drive
PlotROCSaveJPG(rocobj_marsG, path, "MARS_Grid", "MARS_Grid" )
...
### Step 6.2 plot ROC 
rocobj_gbm<- roc(model_gbm$pred$obs, model_gbm$pred$Tumor, 
                ci=TRUE,
                plot=TRUE, 
                legacy.axes=TRUE, percent=TRUE,
                main="Random Forest",
...
confusionMatrix(reference = testData$class, data = predicted_gbm, mode='everything', positive='Tumor')

### save the ROC with svmRadiacl plot to local drive
PlotROCSaveJPG(rocobj_gbm, path, "GBM_tune", "GBM" )
...
### save the ROC with svmRadiacl plot to local drive
PlotROCSaveJPG(rocobj_rf, path, "RandomForest_tune5", "Random Forest" )
...
## specify output folder to save plots
path <- "D:/miRNA_Prj/plots/"
...
### save the ROC with svmRadiacl plot to local drive
PlotROCSaveJPG(rocobj_knn, path, "KNN_tune5", "KNN" )
